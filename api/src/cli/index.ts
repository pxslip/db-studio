...
export async function createCli(): Promise<Command> {
	...
	program.name('db-studio').usage('[command] [options]');
	program.version(pkg.version, '-v, --version');

	program.command('start').description('Start the DB Studio API').action(startServer);
	program.command('init').description('Create a new DB Studio Project').action(init);
