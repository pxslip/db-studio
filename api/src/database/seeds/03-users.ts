...
export default {
	...
	columns: {
		...
		email: { type: 'string', length: 128, nullable: false, unique: true },
		...
		tags: { type: 'json' },
		avatar: { type: 'uuid' },
		language: { type: 'string', length: 8, default: 'en-US' },
		theme: { type: 'string', length: 20, default: 'auto' },
		tfa_secret: { type: 'string', length: 255 },
		status: { type: 'string', length: 16, nullable: false, default: 'active' },
		role: { type: 'uuid', nullable: true, references: { table: 'roles', column: 'id' } },
		token: { type: 'string', length: 255 },
		last_access: { type: 'timestamp' },
		last_page: { type: 'string', length: 255 },
	},
} as TableSeed;
