...
> Relationships are a crucial part of any relational database. d9 supports all standard relationship types, as
...
least once. This is because you must understand how M2Os work in d9 to understand O2Ms, you must understand M2Os
...
no-code configuration. d9 _does not_ enforce opinionated schemas, rule systems, or other arbitrary limitations to
...
- What kinds of relationships exist within d9.
...
By the end, you'll understand everything needed to start building data models in d9, even if relational data model
concepts are a new concept to you.

### d9 vs Classic Data Model Terms

When we use classic data model terms, _such as data table, column, row, etc..._ this signals that the explanation is
focused strictly on what happens in the database. When d9 terminology is used, _such as collection, field, item,
etc..._ this signals that the explanation includes d9 logic and functionality.
...
- When an M2O relational field is configured in d9, an Item Page on the parent collection will enable access to
...
However, in the d9 Data Studio, an M2O field does not automatically provide access to the parent collection's
...
of the [M2O](#many-to-one-m2o) section, we learned that configuring an M2O in d9 does not let us access related
items within an Item Page on the related collection. In d9, configuring an O2M creates an
...
Note the following points from the schema above. When we create an O2M in d9:
...
d9 does not include a dedicated One-to-One (O2O) relationship type or Interface. However, in the database, O2O is
...
Note the following points from the schema above. When we create an O2O in d9:

- We can add the O2O field on either collection. However, in some cases it is more efficient to add it to a specific
  collection.
- Since the O2O field is really just an M2O field behind the scenes, and since d9 doesn't automatically display
...
Note the following points from the schema above. When we create an M2M in d9:
...
- Assuming the M2M alias field is created within the `recipes` collection, d9 does not automatically add a field
...
collection to any item in any collection in the database. When you configure an M2A in d9, an M2A
...
Note the following points from the schema above. When we create an M2A in d9:
...
d9 provides this special relational Interface designed specifically to handle translations. When you create a
